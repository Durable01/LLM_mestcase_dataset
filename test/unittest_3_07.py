...
class TestCase_03_07(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_07.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        component_id = "rob_stopwords_interference"
        #定位输入用例目录
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_07('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = RobStopwordsInterference(LogManager(_isdebug=input_isdebug))   
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_7.txt")
    TestCase_03_07.exec(2,result_path)