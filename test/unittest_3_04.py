...
class TestCase_03_04(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_04.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        component_id = "acc_entity_replace"
        #定位输入用例目录
        input_dir = os.path.join(base_dir, "test","input",component_id)

        #定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test","expect",component_id)

        #最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir,"test","output",component_id)
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_04('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite

    def __init__(self,methodName,input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir) :
        super().__init__(methodName)
        #初始化传入参数，这也意味着要求测试方法必须通过suite组织自行实例化
    ...
    def test_main(self):
        '''单元测试->目标组件3.18：推理链关系的陈述内容生成-覆盖推理可靠性
            1.生成内容并提取日志
            2.保存输出结果
            3.验证输出结果
        '''    
        ...
        if error_log:
            for line in error_log:
                try:
                    validate(instance=json.loads(line),schema=self.expect_error) #之前只是把文本行读进来了，所以得转换
                except ValidationError as e:
    ...
    def config_set(self): 
        ...
        #检查是否有额外配置或设置stack字段
        if "GET_SIMILAR_ENTITIES_ERROR" in self.input_data:
            stk.GET_SIMILAR_ENTITIES_ERROR = self.input_data["GET_SIMILAR_ENTITIES_ERROR"]
        else:
            stk.GET_SIMILAR_ENTITIES_ERROR = False
        if "CHECK_KG_CONSISTENCY_ERROR1" in self.input_data:
            stk.CHECK_KG_CONSISTENCY_ERROR1= self.input_data["CHECK_KG_CONSISTENCY_ERROR1"]
        else:
            stk.CHECK_KG_CONSISTENCY_ERROR1 = False
        if "CHECK_KG_CONSISTENCY_ERROR2" in self.input_data:
            stk.CHECK_KG_CONSISTENCY_ERROR2= self.input_data["CHECK_KG_CONSISTENCY_ERROR2"]
        else:
            stk.CHECK_KG_CONSISTENCY_ERROR2 = False
    ...
    def get_choices(self):
        ...
        try:
            ...
            input_knowentity = self.input_data["knowentity"]
            input_entityrelation = self.input_data["entityrelation"]
            input_isdebug = self.input_data["isdebug"]
        except Exception as e:
            self.skipTest("input_data:"+"输入样例必要字段缺失:"+str(e))
        #生成内容
        self.generator = AccEntityReplace(LogManager(_isdebug=input_isdebug))   
        result_choices = self.generator.generate_choice(input_knowid,input_knowstr,input_knowcata,input_knowentity, input_entityrelation)
    ...
    def __str__(self):
        # 这样在TextTestRunner的报告里会显示参数,从而相区分
        return f"{self.__class__.__name__}.{self._testMethodName} (file_path={self.input_path})"

if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_4.txt")
    TestCase_03_04.exec(2,result_path)