...
class TestCase_03_18(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_18.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        #初始目录待修改部分--------------------------------------------------------------------Begin
        #定位输入用例目录
        input_dir = os.path.join(base_dir, "test","input","inf_inference_chain")

        #定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test","expect","inf_inference_chain")

        #最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir,"test","output","inf_inference_chain")
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_18('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite
    ...
    def config_set(self): 
        ...
        #检查是否有额外配置或设置stack字段
        if "get_n_step_consequence_n_2" in self.input_data:
            stk.GET_N_STEP_CONSEQUENCE_N_2 = self.input_data["get_n_step_consequence_n_2"]
        if "get_n_step_consequence_n_3" in self.input_data:
            stk.GET_N_STEP_CONSEQUENCE_N_3 = self.input_data["get_n_step_consequence_n_3"]
        if "get_n_step_consequence_error" in self.input_data:
            stk.GET_N_STEP_CONSEQUENCE_ERROR = self.input_data["get_n_step_consequence_error"]
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = InfInferenceChain(LogManager(_isdebug=input_isdebug))   
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_18.txt")
    TestCase_03_18.exec(2,result_path)