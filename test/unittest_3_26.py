...
from src.att_relation_completeness import AttRelationCompleteness

class TestCase_03_26(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试'''
        suite = TestCase_03_26.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ''' 构建测试套件 '''
        input_dir = os.path.join(base_dir, "test", "input", "att_relation_completeness")
        expect_dir = os.path.join(base_dir, "test", "expect", "att_relation_completeness")
        output_dir = os.path.join(base_dir, "test", "output", "att_relation_completeness")
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(input_paths, expect_choice_paths, expect_error_paths, expect_run_paths):
            suite.addTest(TestCase_03_26('test_main', input_path, expect_choice_path, expect_error_path, expect_run_path, output_dir))
        return suite
    ...
    def config_set(self):
        ...
        if "get_parallel_head_error" in self.input_data:
            stk.GET_PARALLEL_HEAD_ERROR = self.input_data["get_parallel_head_error"]
    ...
    def get_choices(self):
        ...
        # 调用组件
        self.generator = AttRelationCompleteness(LogManager(_isdebug=input_isdebug))
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_26.txt")
    TestCase_03_26.exec(2, result_path)
