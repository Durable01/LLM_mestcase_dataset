...
class TestCase_03_15(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试
        '''
        suite = TestCase_03_15.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ...
        # 初始目录待修改部分--------------------------------------------------------------------Begin
        # 定位输入用例目录
        input_dir = os.path.join(base_dir, "test", "input", "lim_unit_transform")

        # 定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test", "expect", "lim_unit_transform")

        # 最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir, "test", "output", "lim_unit_transform")
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(input_paths, expect_choice_paths,
                                                                                      ...
                                                                                      expect_run_paths):
            suite.addTest(
                TestCase_03_15('test_main', input_path, expect_choice_path, expect_error_path, expect_run_path,
                               output_dir))
        return suite
    ...
    def setUp(self):
        ...
        # 1.读取测试用例文件
        print(f"[{time.asctime()}]Test case {self.input_label} started...")
        # 读取加载json文件
    ...
    def test_main(self):
        ...
        print(f"Generation time: {generate_time}")
    ...
    def get_choices(self):
        ...
        try:
            ...
            input_inference = self.input_data["inference"]
            input_isdebug = self.input_data["isdebug"]
        except Exception as e:
            self.skipTest("input_data:" + "输入样例必要字段缺失:" + str(e))
        # 生成内容
        self.generator = LimUnitTransform(LogManager(_isdebug=input_isdebug))
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_15.txt")
    TestCase_03_15.exec(2, result_path)