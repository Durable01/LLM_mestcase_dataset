...
class TestCase_03_12(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试
        '''
        suite = TestCase_03_12.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ...
        # 初始目录待修改部分--------------------------------------------------------------------Begin
        # 定位输入用例目录
        input_dir = os.path.join(base_dir, "test", "input", "cons_reload_parallelrelation")

        # 定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test", "expect", "cons_reload_parallelrelation")

        # 最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir, "test", "output", "cons_reload_parallelrelation")
        ...
        # 并行遍历各个相对应的文件
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(input_paths, expect_choice_paths,
                                                                                      expect_error_paths,
                                                                                      expect_run_paths):
            suite.addTest(
                TestCase_03_12('test_main', input_path, expect_choice_path, expect_error_path, expect_run_path,
                               output_dir))
        return suite
    ...
    def test_main(self):
        ...
        # 生成日志结果从文件读取
        with open(self.error_log_path, "r", encoding="UTF-8") as f1, open(self.run_log_path, "r",
        ...
        if not self.is_sameExist(result_choices, self.expect_choice) or not self.is_sameExist(error_log,
                                                                                              self.expect_error) or not self.is_sameExist(
                run_log, self.expect_run):
    ...
    def get_choices(self):
        ...
        try:
            ...
            input_parallelrelation = self.input_data["parallelrelation"]
            input_isdebug = self.input_data["isdebug"]
        except Exception as e:
            self.skipTest("input_data:" + "输入样例必要字段缺失:" + str(e))
        # 生成内容
        self.generator = ConsReloadParallelRelation(LogManager(_isdebug=input_isdebug))
        result_choices = self.generator.generate_choice(input_knowid, input_knowstr, input_knowcata, input_parallelrelation)
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_12.txt")
    TestCase_03_12.exec(2, result_path)