...
from src.att_value_judge import AttValueJudge

class TestCase_03_28(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试
        '''
        suite = TestCase_03_28.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ...
        '''
        # 输入用例目录
        input_dir = os.path.join(base_dir, "test", "input", "att_value_judge")
        # 预期输出目录
        expect_dir = os.path.join(base_dir, "test", "expect", "att_value_judge")
        # 实际输出目录
        output_dir = os.path.join(base_dir, "test", "output", "att_value_judge")
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(input_paths, expect_choice_paths, expect_error_paths, expect_run_paths):
            suite.addTest(TestCase_03_28('test_main', input_path, expect_choice_path, expect_error_path, expect_run_path, output_dir))
        return suite
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = AttValueJudge(LogManager(_isdebug=input_isdebug))
...
if __name__ == '__main__':
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_28.txt")
    TestCase_03_28.exec(2, result_path)
