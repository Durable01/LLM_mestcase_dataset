...
class TestCase_03_08(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试'''
        suite = TestCase_03_08.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(
            ...
            suite.addTest(TestCase_03_08('test_main', input_path, expect_choice_path, expect_error_path,
    ...
    def get_choices(self):
        ''' 组件调用 generate_choice 所需要的参数的设置区域。
            不同的组件需要对该函数进行调整。
        '''
        # 提取必需字段
        try:
        ...
        # 生成内容
        self.generator = RobHomophoneDisturb(LogManager(_isdebug=input_isdebug))
    ...
    def __str__(self):
        # 这样在 TextTestRunner 的报告里会显示参数,从而相区分
        return f"{self.__class__.__name__}.{self._testMethodName} (file_path={self.input_path})"

if __name__ == "__main__":
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_8.txt")
    TestCase_03_08.exec(2, result_path)