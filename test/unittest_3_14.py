...
class TestCase_03_14(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_14.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        #初始目录待修改部分--------------------------------------------------------------------Begin
        #定位输入用例目录
        input_dir = os.path.join(base_dir, "test","input","lim_valueconstrain_transform")

        #定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test","expect","lim_valueconstrain_transform")

        #最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir,"test","output","lim_valueconstrain_transform")
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_14('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite
    ...
    def test_main(self):
        '''单元测试->目标组件3.14：数值约束的条件更改-覆盖条件敏感性
            1.生成内容并提取日志
            2.保存输出结果
            3.验证输出结果
        '''    
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = LimValueConstraintTransform(LogManager(_isdebug=input_isdebug)) 
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_14.txt")
    TestCase_03_14.exec(2,result_path)