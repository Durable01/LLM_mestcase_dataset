...
class TestCase_03_09(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_09.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        #初始目录待修改部分--------------------------------------------------------------------Begin
        #定位输入用例目录
        input_dir = os.path.join(base_dir, "test","input","rob_synonym_algorithm")

        #定位预期输出用例目录
        expect_dir = os.path.join(base_dir, "test","expect","rob_synonym_algorithm")

        #最后用来存放实际输出结果的目录
        output_dir = os.path.join(base_dir,"test","output","rob_synonym_algorithm")
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_09('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite
    ...
    def config_set(self): 
        ...
        if "GET_ENTITES_ERROR" in self.input_data:
            stk.GET_ENTITES_ERROR = self.input_data["GET_ENTITES_ERROR"]
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = RobSynonymAlgorithm(LogManager(_isdebug=input_isdebug))
        result_choices = self.generator.generate_choice(input_knowid,input_knowstr,input_knowcata,input_knowentity)
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_9.txt")
    TestCase_03_09.exec(2,result_path)