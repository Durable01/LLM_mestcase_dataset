...
class TestCase_03_05(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        """执行测试"""
        suite = TestCase_03_05.build_suite()
        if test_result_path:
    ...
    def build_suite():
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(input_paths, expect_choice_paths, expect_error_paths, expect_run_paths):
            suite.addTest(TestCase_03_05('test_main', input_path, expect_choice_path, expect_error_path, expect_run_path, output_dir))
        return suite
    ...
    def config_set(self):
        ...
        if "GET_SIMILAR_ENTITIES_ERROR" in self.input_data:
            stk.GET_SIMILAR_ENTITIES_ERROR = bool(self.input_data["GET_SIMILAR_ENTITIES_ERROR"])
        else:
...
if __name__ == "__main__":
    ...
    TestCase_03_05.exec(2, result_path)