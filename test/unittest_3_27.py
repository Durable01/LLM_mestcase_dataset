...
from src.att_relation_independence import AttRelationIndependence

class TestCase_03_27(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试'''
        suite = TestCase_03_27.build_suite()
        if test_result_path:
    ...
    def build_suite():
        '''构建测试套件'''
        input_dir = os.path.join(base_dir, "test", "input", "att_relation_independence")
        expect_dir = os.path.join(base_dir, "test", "expect", "att_relation_independence")
        output_dir = os.path.join(base_dir, "test", "output", "att_relation_independence")
        ...
        for input_path, expect_choice_path, expect_error_path, expect_run_path in zip(
            ...
            suite.addTest(TestCase_03_27(
                'test_main',
                input_path, expect_choice_path, expect_error_path, expect_run_path, output_dir))
        return suite
    ...
    def get_choices(self):
        ...
        # 调用组件
        self.generator = AttRelationIndependence(LogManager(_isdebug=input_isdebug))
...
if __name__ == '__main__':
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_27.txt")
    TestCase_03_27.exec(2, result_path)
