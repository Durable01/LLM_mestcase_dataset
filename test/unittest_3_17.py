...
class TestCase_03_17(unittest.TestCase):

    @staticmethod
    def exec(vision = 1, test_result_path = None):
        '''执行测试
        '''
        suite = TestCase_03_17.build_suite()
        if test_result_path :
    ...
    def build_suite():
        ...
        component_id = "inf_single_inference"
        #定位输入用例目录
        ...
        for input_path,expect_choice_path,expect_error_path,expect_run_path in zip(input_paths,expect_choice_paths,expect_error_paths,expect_run_paths):
            suite.addTest(TestCase_03_17('test_main',input_path,expect_choice_path,expect_error_path,expect_run_path,output_dir))
        return suite
    ...
    def config_set(self): 
        ...
        #检查是否有额外配置或设置stack字段
        if "tempstk_inference_consistency_return" in self.input_data:
            stk.TEMPSTK_INFERENCE_CONSISTENCY_RETURN = self.input_data["tempstk_inference_consistency_return"]
        else:
            stk.TEMPSTK_INFERENCE_CONSISTENCY_RETURN = True
        if "tempstk_inference_consistency_error" in self.input_data:
            stk.TEMPSTK_INFERENCE_CONSISTENCY_ERROR= self.input_data["tempstk_inference_consistency_error"]
        else:
            stk.TEMPSTK_INFERENCE_CONSISTENCY_ERROR = False
    ...
    def get_choices(self):
        ...
        #生成内容
        self.generator = InfSingleInference(LogManager(_isdebug=input_isdebug))   
...
if __name__ == "__main__":
    result_path = os.path.join(base_dir,"test","output","unittest_3_17.txt")
    TestCase_03_17.exec(2,result_path)