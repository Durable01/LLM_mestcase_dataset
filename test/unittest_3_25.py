...
from src.att_relation_domain import AttRelationDomain

class TestCase_03_25(unittest.TestCase):

    @staticmethod
    def exec(vision=1, test_result_path=None):
        '''执行测试'''
        suite = TestCase_03_25.build_suite()
        if test_result_path:
    ...
    def build_suite():
        '''构建测试套件'''
        input_dir = os.path.join(base_dir, "test", "input", "att_relation_domain")
        expect_dir = os.path.join(base_dir, "test", "expect", "att_relation_domain")
        output_dir = os.path.join(base_dir, "test", "output", "att_relation_domain")
    ...
    def get_choices(self):
        ...
        # 调用组件
        self.generator = AttRelationDomain(LogManager(_isdebug=input_isdebug))
...
if __name__ == '__main__':
    result_path = os.path.join(base_dir, "test", "output", "unittest_3_25.txt")
    TestCase_03_25.exec(2, result_path)
